...
class Mamba4POI(SequentialRecommender):
    ...
    def sinusoidal_position_encoding(self,locations,locdim,device):
        ...
        for i in range(0, d, 2):
            position_encoding[:, i] = torch.sin(locations[:,0] * (-i / d)**6)
            position_encoding[:,i + 1] = torch.cos(locations[:,0] * (-(i + 1) / d)**6)
            position_encoding[:, d+i] = torch.sin(locations[:,1] * (-i / d)**3)
            position_encoding[:,d+i + 1] = torch.cos(locations[:,1] *(-(i + 1) / d)**6)
    ...
    def sinusoidal_time_encoding(self,TimeTensor,TimeDim,Term='day'):
        ...
        sin_encoding = torch.sin(TimeTensor[:, :, 0:1] * (-i[::2] / d)**6)
        cos_encoding = torch.cos(TimeTensor[:, :, 0:1] * (-i[1::2] / d)**6)
