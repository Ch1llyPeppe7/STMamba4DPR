...
class Mamba4POI(SequentialRecommender):
    ...
    def sinusoidal_position_encoding(self,locations,locdim,device):
        ...
        for i in range(0, d, 2):
            position_encoding[:, i] = torch.sin(locations[:,0] * torch.exp(torch.log(torch.tensor(10000))*(i / d)))
            position_encoding[:,i + 1] = torch.cos(locations[:,0]*torch.exp(torch.log(torch.tensor(10000)*((i + 1) / d))))
            position_encoding[:, d+i] = torch.sin(locations[:,1]*torch.exp(torch.log(torch.tensor(10000))*(i / d)))
            position_encoding[:,d+i + 1] = torch.cos(locations[:,1]*torch.exp(torch.log(torch.tensor(10000)*((i + 1) / d))))
    ...
    def sinusoidal_time_encoding(self,TimeTensor,TimeDim,Term='day'):
        ...
        sin_encoding = torch.sin(TimeTensor[:, :, 0:1]* torch.exp(torch.log(torch.tensor(10000))*(i[::2] / d)))
        cos_encoding = torch.cos(TimeTensor[:, :, 0:1]* torch.exp(torch.log(torch.tensor(10000))*(i[1::2] / d)))
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == "BPR":
            ...
            logits = torch.matmul(item_output, 
                        test_item_emb.transpose(0, 1))#+(pos_time*item_output).sum(1).unsqueeze(1)
    ...
    def predict(self, interaction):
        ...
        scores = torch.mul(item_output, test_item_emb).sum(dim=1)#+(pos_time*item_output).sum(1).unsqueeze(1)  # [B]
        return scores
