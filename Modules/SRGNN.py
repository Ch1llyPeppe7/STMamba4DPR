...
class Model(nn.Module):
    ...
    def __init__(self, n_items, hist_length, emb_size=128, step=1, device='cpu'):
    ...
    def forward(self, item_seq):
        ...
        seq_hidden = torch.gather(hidden, dim=1, index=alias_inputs)  # batch x hist x emb
        ht = seq_hidden[:, -1, :]  # last item's embedding
        ...
        seq_output = self.linear_transform(torch.cat([a, ht], dim=1))  # batch x emb

        all_item_index = torch.arange(0, self.n_items+1).to(self.device)  # all POI nodes [0,n_item] including padding index 0
        all_node_embs = self.item_embedding(all_item_index)
        all_scores = torch.matmul(seq_output, torch.transpose(all_node_embs, 0, 1))
        return all_scores
